// CACHE_BUST_VERSION: 20260711221949
(function () {
  "use strict";
  ...
  var FALLBACK_SITE_DATA = {
    siteSettings: {
      ...
      philosophy: "数据源未就绪。正式内容只应来自 assets/data/site-data.json 或你明确选择的本机草稿。",
      email: "",
      portfolioPdf: "",
      accentColor: "#536A63",
      hero: {
        sealSuffix: "DATA SOURCE REQUIRED",
        primaryAction: {
          labelCN: "检查数据",
          labelEN: "Check Data",
        ...
        bottomStrip: ["NO OFFICIAL DATA", "CHECK SITE-DATA.JSON", "FALLBACK BLOCKED FROM EXPORT"]
      },
    ...
    navigation: [
      { id: "works", labelCN: "数据检查", labelEN: "Data Check", href: "#atelier-console" }
    ],
    methods: [],
    projects: [
      {
        id: "data-not-loaded",
        title: "站点数据未读取",
        titleEN: "Site data not loaded",
        category: "System",
        year: "—",
        location: "",
        status: "Fallback / Not official",
        material: "",
        scale: "",
        role: "diagnostic placeholder",
        cover: "",
        excerpt: "这是诊断占位卡片，不是正式项目。请确认 assets/data/site-data.json 可以读取。",
        description: "正式项目数据未读取成功。请检查 JSON 路径、JSON 格式，或在后台导入完整 site-data.json。",
        tags: ["data", "json", "fallback"],
        assets: [],
        article: createDefaultArticleContent()
      }
    ]
  }
  ...
  var navigation = [
    ...
    { labelCN: "作品", labelEN: "Works", href: "#atelier-console" },
    ...
    { labelCN: "营造", labelEN: "Studio", href: "#atelier-console" },
  ...
  function createEmergencyFallbackSiteData() {
    return {
      siteSettings: {
        studioName: "站点数据未加载",
        studioSeal: "DATA",
        taglineCN: "assets/data/site-data.json 未能读取。",
        taglineEN: "Site data is not loaded.",
        intro: "请检查 site-data.json 是否存在、JSON 格式是否有效，以及当前预览方式是否允许读取本地数据文件。",
        philosophy: "",
        email: "",
        portfolioPdf: "",
        accentColor: "#536A63",
  ...
  function applySafeFallbackAfterDataFailure(message) {
    ...
    /*
      V44:
      If official JSON cannot be read, keep built-in fallback only to prevent
      a blank page. It cannot be exported as official data.
    */
  ...
  async function loadOfficialSiteData() {
    ...
    applySafeFallbackAfterDataFailure("正式数据文件读取失败，已临时使用诊断占位数据，避免旧版内容误导。尝试路径：" + errors.join("；"));
    return false;
  }
  ...
  function exportFullSiteData() {
    if (state.dataSource === "fallback") {
      var message = "当前页面正在使用内置备用数据，不是正式 site-data.json，也不是你明确选择的本机草稿。\n\n已阻止导出，避免把备用内容误当正式数据。\n\n请先确认 assets/data/site-data.json 可读取，或点击“使用本机草稿”后再导出。";
  ...
  function ensureProjectsNotEmpty() {
    ...
    if (FALLBACK_SITE_DATA && Array.isArray(FALLBACK_SITE_DATA.projects) && FALLBACK_SITE_DATA.projects.length) {
      projects = FALLBACK_SITE_DATA.projects.map(normalizeProject);
      state.projects = projects.map(normalizeProject);
      if (!state.dataLoadError) {
        state.dataLoadError = "未读取到项目数据，已临时使用内置备用项目。";
      }
    }
  }
    ...
    if (status) {
      ...
      var label = state.dataSource === "local"
        ...
        : state.dataSource === "fallback"
          ? (isFileMode ? "当前使用：诊断占位数据。file:/// 直接预览不能可靠读取 JSON；不会再显示旧版首页文字，请用 Live Server / GitHub Pages / 导入 JSON。" : "当前使用：诊断占位数据。正式 JSON 未读取成功；不会再显示旧版首页文字，也已禁止导出 fallback。")
