...
(function () {
  ...
  function createContactCurtain(canvas, stage, rawText) {
    ...
    function readArchitectureGeometry() {
    ...
    function buildStrands() {
      ...
      var target = mobile
        ? Math.max(14, Math.min(20, Math.round(availableWidth / 28)))
        : Math.max(24, Math.min(34, Math.round(availableWidth / 26)));
      var charCursor = 0;
      var beadCount = 0;
      var positions = [];
      for (var i = 0; i < target; i += 1) {
        var t = target === 1 ? 0.5 : i / (target - 1);
        var jitter = (seeded(i + 19) - 0.5) * (mobile ? 0.018 : 0.014);
      ...
      positions.forEach(function (t, s) {
        ...
        var top = anchorY + (seeded(s + 31) - 0.5) * (mobile ? 8 : 12);
        var bottomAllowance = mobile ? 76 : 66;
        var availableHeight = Math.max(150, height - top - bottomAllowance);
        var lengthFactor = 0.62 + seeded(s + 73) * 0.34 + centerWeight * 0.18;
        if (s % 6 === 0 || s % 11 === 0) lengthFactor += 0.12;
        if (s % 8 === 0) lengthFactor -= 0.06;
        var strandLength = Math.max(120, availableHeight * lengthFactor);
        ...
        for (var b = 0; b < beadTotal; b += 1) {
          ...
          var visible = !(b > 2 && gapSeed < (depth > 0.75 ? 0.08 : 0.03));
    ...
    function applyImpulse(x, y, vx, vy, pointerType) {
      ...
      var radius = mobile ? 106 : 142;
      var speed = Math.min(42, Math.sqrt(vx * vx + vy * vy));
      ...
      strands.forEach(function (strand, strandIndex) {
        strand.beads.forEach(function (bead, beadIndex) {
          ...
          var mobility = 0.22 + Math.pow(depth, 0.84) * 0.98;
          var influence = Math.pow(1 - distance / radius, 1.9) * mobility;
          var side = Math.abs(dx) > 2 ? Math.sign(dx) : (vx >= 0 ? 1 : -1);
          var force = (6.4 + speed * 0.26) * influence;
          var forceX = side * force + vx * 0.18 * influence;
          var forceY = -(1.25 + Math.abs(vx) * 0.04) * influence + vy * 0.03 * influence;
          ...
          for (var offset = 1; offset <= 4; offset += 1) {
            ...
            [beadIndex - offset, beadIndex + offset].forEach(function (neighborIndex) {
              ...
              neighbor.vx += forceX * falloff * 0.72;
              neighbor.vy += forceY * falloff * 0.26;
              neighbor.glow = Math.min(0.78, neighbor.glow + influence * falloff);
            });
          }

          [-2, -1, 1, 2].forEach(function (strandOffset) {
            var coupling = Math.pow(0.46, Math.abs(strandOffset));
            transferImpulse(strandIndex + strandOffset, beadIndex, forceX, forceY * 0.52, coupling * influence);
          });
        });
      });
    ...
    function updateStrand(strand, strandIndex) {
      ...
      var damping = 0.906;
      var restSpringX = 0.024;
      var restSpringY = 0.066;
      var stringSpring = 0.124;
      ...
      for (var i = 1; i < beads.length; i += 1) {
        ...
        var topStiffness = 1.04 + (1 - depth) * 1.9;
        ...
        if (neighborCount) bead.vx += ((neighborX / neighborCount) - bead.x) * 0.026;
    ...
    function eventPoint(event) {
      ...
      if ((event.type === "pointermove" || event.type === "mousemove" || event.type === "touchmove") && now - lastPointer.handled < 22) {
        return;
      }
      var rect = stage.getBoundingClientRect();
    ...
    stage.__contactCurtainDebug = function () {
      return {
        ...
        anchorY: Number(anchorY.toFixed(1)),
        inViewport: inViewport,
        running: metrics.running,
        reducedMotion: reducedMotion,
        renderer: "canvas-real-text-curtain-v73-balanced"
      };
    };

    if (architecture && !architecture.complete) architecture.addEventListener("load", resize, { once: true });

    if (typeof ResizeObserver !== "undefined") {
      resizeObserver = new ResizeObserver(resize);
      resizeObserver.observe(stage);
      if (architecture) resizeObserver.observe(architecture);
    } else {
    ...
    return {
      destroy: function () {
        ...
        stage.classList.remove("is-curtain-ready");
      },
      getMetrics: function () {
